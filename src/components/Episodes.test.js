...
test("renders Episodes List without errors", () => {
  render(<Episodes episodes={[]} />);
});

test("Component renders and rerenders with props which is a list of episodes", () => {
  const { rerender, queryAllByTestId } = render(
    <Episodes episodes={[]} />
  );

  expect(queryAllByTestId(/shows/i)).toStrictEqual([]);

  rerender(<Episodes episodes={shows} />);

  const episodes = queryAllByTestId(/shows/i);
